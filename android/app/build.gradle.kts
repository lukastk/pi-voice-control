plugins {
    id("com.android.application")
    id("org.jetbrains.kotlin.android")
}

android {
    namespace = "com.voiceagentbridge"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.voiceagentbridge"
        minSdk = 26
        targetSdk = 34
        versionCode = 1
        versionName = "0.1.0"
    }

    buildTypes {
        debug {
            isMinifyEnabled = false
            applicationIdSuffix = ".debug"
            isDebuggable = true
        }
        release {
            isMinifyEnabled = false
            isDebuggable = false
            // No proguard config — sideloaded personal app, codebase is tiny.
        }
    }

    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }

    kotlinOptions {
        jvmTarget = "17"
    }
}

dependencies {
    implementation("androidx.appcompat:appcompat:1.7.0")
    implementation("androidx.core:core-ktx:1.13.1")
    // Native LiveKit SDK so audio plays through STREAM_VOICE_CALL +
    // MODE_IN_COMMUNICATION and survives screen-off. We can't make the
    // WebView do this from outside — Chromium hardcodes USAGE_MEDIA +
    // STREAM_MUSIC, which the OS pauses on visibility change.
    implementation("io.livekit:livekit-android:2.25.1")
}
